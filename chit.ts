...
function dataImg(config: Config, _avatars: any, url: string) {
  return img({
    height: 32,
...
const flowdockAvatar = (config: Config, avatars: void, t: Tri<FlowdockUser>) =>
  tri(t, fd =>
...
const githubAvatar = (config: Config, avatars: void, t: Tri<GithubUser>) =>
  tri(t, gh =>
  ...
  tr({ style: contactMatchesStyle(contact, needle) }, [
    td(
      a(
        ...
        dataImg(config, avatars, contact.thumb),
      ),
    ),
    td(
      a(
...
const renderTable = (
  ...
  table([
    thead(tableHeader),
    tbody(contacts.map(renderRow(config, avatars, needle, firstNameOnly))),
  ])
...
function render(
  ...
  return iddqd
    ...
    : renderContacts(config, contacts, avatars, needle, firstNameOnly)
}
...
function codeSource(
  ...
  return keyPresses
    ...
    .map(c => {
      if (c === code[index++] && index === code.length) {
        return true
      } else {
        index = 0
        return false
...
function main(responses: Sources) {
  ...
  const config$ = responses.HTTP.select("config").flatten().map(res => res.body)
  ...
  const avatars$ = xs.of(null)

  const vtree$ = xs
    .combine(config$, contacts$, avatars$, filter$, iddqd$, idclip$)
    .map(render)
